---
title: 'Lab 1: Intro to Data Summaries and Graphics in R'
...
To view the names of the variables and dimension of the \texttt{nhanes} data frame type the following commands:
...
You could also look at all of the data frame at once by typing its name into the console, but that might be unwise here. We know `nhanes` has 1500 rows, so viewing the entire data set would mean flooding your screen. It’s better to take small peeks at the data with `head()`\
...
Next, to make a bar plot of the entries in the table, put the table inside the 
...
concept: R commands can be nested. You could also break this into two steps by 
...
Last, scatterplots are used to show the relationship between two numeric variables.  To make a scatterplot use the \texttt{plot()} function.
