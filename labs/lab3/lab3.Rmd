...
In this lab we will go over how to perform a hypothesis test and compute a confidence interval for the difference between two means.  We will use a data set on birth records to answer the question: "Is there convincing evidence that newborns from mothers that smoke have a different average birth weight than newborns from mothers who don't smoke?"
...
The box plots and summary statistics indicate that babies from mothers that smoke tend to weigh less than babies from mother that do not smoke.  But is the difference statistically significant?  That is, are the differences in weight due to an actual effect from smoking or random sampling variability? 

We can use the \texttt{t.test()} function to perform the hypothesis test and calculate a confidence interval for the difference between the two means.  Specifically, we test:

$H_0: \mu_{ns} = \mu_{s}$ (the mean weight of babies from nonsmoking mothers is the same as the mean weight of babies from smoking mothers)

$H_A: \mu_{ns} \neq \mu_{s}$ (the mean weight of babies from nonsmoking mothers is different than the mean weight of babies from smoking mothers)

<!-- Here $\mu_{ns}$ is the population mean weight of babies for nonsmoking mothers, and $\mu_{s}$ is the population mean weight of babies for smoking mothers. -->
...
Since the p-value $<0.05$, we reject $H_0$.  Therefore, there is a statistically significant difference in birth weight.  The data provide convincing evidence that the mean birth weight of babies from smoking mothers is less than the mean birth weight of babies from nonsmoking mothers. 

The `t.test()` function also computes a 95\% confidence interval for difference between the two means, $\mu_{ns} - \mu_s$.  We are 95\% that the average birth weight of babies from nonsmoking mothers is between 0.052 and 0.58 pounds more than the average birth weight of babies from smoking mothers.

Similar to the `boxplot()` function, the `t.test()` function also uses the formula notation `y ~ x` where `y` is a numerical variable, and `x` is a categorical variable specifying the two groups.
